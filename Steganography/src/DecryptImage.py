...
import numpy as np
from src.DecryptText import Decrypt
...
def decrypt():
    ...
    for i in range(int((len(data)+1)/8)):
        encrypted_message.append(data[i*8:(i*8+8)])
    # join all the letters to form the message.
    encrypted_message = [chr(int(''.join(i), 2)) for i in encrypted_message]
    encrypted_message = ''.join(encrypted_message)
    dec = Decrypt(encrypted_message)
    message = dec.decrypt()
    message_label = Label(app, text=message, bg='lavender', font=("Times New Roman", 10))
    message_label.place(x=300, y=500)
